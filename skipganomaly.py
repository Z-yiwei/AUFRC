...
from networks import NetD, weights_init, define_G, define_D, get_scheduler
...
def evaluate_all(labels, scores):
    ...
    scores = torch.where(scores >= 0.2, torch.tensor(1), torch.tensor(0))
    ...
    return [auc_, auprc_, acc, precision, recall, f1_score_]
...
class Skipganomaly(BaseModel):
    ...
    def __init__(self, opt, data=None):
        ...
        self.netd = define_D(self.opt, norm='batch', use_sigmoid=True, init_type='normal')

        ##
        if self.opt.resume != '':
            print("\nLoading pre-trained networks.")
            self.opt.iter = torch.load(os.path.join(self.opt.resume, '240G_0.pth'))['epoch']
            self.netg.load_state_dict(torch.load(os.path.join(self.opt.resume, '240G_0.pth'))['state_dict'])
            self.netd.load_state_dict(torch.load(os.path.join(self.opt.resume, '240D_0.pth'))['state_dict'])
            print("\tDone.\n")
        ...
        # self.input = torch.empty(size=(self.opt.batchsize, 3, 128, 480), dtype=torch.float32,
        #                          device=self.device)
        # self.noise = torch.empty(size=(self.opt.batchsize, 3, 128, 480), dtype=torch.float32,
        #                          device=self.device)
        # self.label = torch.empty(size=(self.opt.batchsize,), dtype=torch.float32, device=self.device)
        # self.gt = torch.empty(size=(opt.batchsize,), dtype=torch.long, device=self.device)
        # self.fixed_input = torch.empty(size=(self.opt.batchsize, 3, 128, 480),
    ...
    def test(self, plot_hist=True):
        ...
        with torch.no_grad():
            ...
            result_list = evaluate_all(self.gt_labels, self.an_scores)
            print("The result is AUC:{:.3f}  AUPRC: {:.3f}  Accuracy: {:.3f} "
                  "Precision:{:.3f} Recall{:.3f}  F1-score: {:.3f}".format(
                                                                            result_list[0],
                                                                            result_list[1],
                                                                            result_list[2],
                                                                            result_list[3],
            ...
            if plot_hist:
                ...
                sns.histplot(nrm_scr, label=r'Normal Scores', color='b', kde=True,
                             edgecolor='black', bins=np.arange(0, 1.01, 0.01))
                sns.histplot(abn_scr, label=r'Abnormal Scores', color='r', kde=True,
                             edgecolor='black', bins=np.arange(0, 1.01, 0.01))
