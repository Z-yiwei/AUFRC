...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        unet_block = UnetSkipConnectionBlock(output_nc, ngf * 2, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.fc_out(out)
        return out.reshape(B, self.embed_size, H, W)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv = nn.Sequential(nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                                 stride=2, padding=1, bias=use_bias))
        # if inner_nc % 512 == 0:
        #     downconv = nn.Sequential(MultiHeadSelfAttention(embed_size=inner_nc, num_heads=4), nn.Conv2d(input_nc, inner_nc, kernel_size=4,
        #                          stride=2, padding=1, bias=use_bias), MultiSpectralAttentionLayer(channel=inner_nc))
        downconv = nn.Sequential(nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                                           stride=2, padding=1, bias=use_bias))
        ...
        if outermost:
            upconv = nn.Sequential(nn.ConvTranspose2d(inner_nc * 2, outer_nc,
            ...
            upconv = nn.Sequential(nn.ConvTranspose2d(inner_nc, outer_nc,
                                        kernel_size=4, stride=2,
